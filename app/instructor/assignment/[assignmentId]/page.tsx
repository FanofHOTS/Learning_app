...
export default function InstructorAssignmentGradingPage() {
  ...
  async function confirmGrade() {
    ...
    try {
      ...
      setSuccessMessage(
        `Đã chấm bài của ${updated.user_name ?? `học sinh #${updated.user_id}`}.`,
      );
    } catch (error) {
      setErrorMessage(
        error instanceof Error
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Xem bài nộp của học sinh, chấm điểm và phản hồi
            </p>
          </div>
        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <h3 className="text-lg font-semibold text-slate-900">
                          ...
                          {selectedSubmission.user_name ??
                            `học sinh #${selectedSubmission.user_id}`}
                        </h3>
              ...
              <div className="rounded-2xl bg-slate-50 px-4 py-3">
                ...
                <p className="mt-0.5 text-sm font-semibold text-slate-900">
                  {confirmSubmission.user_name ??
                    `Học sinh #${confirmSubmission.user_id}`}
                </p>
              </div>
              <div className="rounded-2xl bg-slate-50 px-4 py-3">
