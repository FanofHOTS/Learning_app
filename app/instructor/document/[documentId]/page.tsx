...
import { ChevronLeft, FileText, LoaderCircle, Menu, XCircle } from "lucide-react";
...
export default function InstructorDocumentDetailPage() {
  ...
  useEffect(() => {
    ...
    async function loadPageData() {
      ...
      try {
        const [fetchedDocument, refComponent] = await Promise.all([
          getDocumentById(documentId),
          getCourseComponentByRef("document", documentId),
        ]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Xem nội dung tài liệu từ khóa học giảng dạy
            </p>
          </div>
        </div>
