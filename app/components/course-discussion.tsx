...
import {
  ...
  getCourseComments,
  updateCourseComment,
} from "../lib/api_course_discussion";
...
export default function CourseDiscussion({
  ...
  function addCommentToTree(
    current: CourseDiscussionComment[],
    newComment: CourseDiscussionComment,
  ...
  function replaceCommentInTree(
    current: CourseDiscussionComment[],
    targetId: number,
    replacement: CourseDiscussionComment,
  ): CourseDiscussionComment[] {
    return current.map((comment) => {
  ...
  const loadComments = useCallback(async () => {
    try {
      setIsLoading(true);
      const fetchedComments = await getCourseComments(courseId);
  ...
  async function handleCreateComment() {
    ...
    const optimisticComment: CourseDiscussionComment = {
      id: optimisticId,
      course_id: courseId,
  ...
  async function handleUpdateComment(commentId: number) {
    ...
    try {
      await updateCourseComment(commentId, newContent);
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <h3 className="text-base font-semibold text-slate-900">Thảo luận khóa học</h3>
        {!isLoading ? (
        ...
        <textarea
          ...
          placeholder={
            replyTo
              ...
              : "Chia sẻ suy nghĩ của bạn về khóa học này..."
          }
          disabled={isSubmitting}
        />
